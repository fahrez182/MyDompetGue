    ...
    <x-slot name="header">
        <div class="max-w-xl mx-auto px-4 sm:px-6 lg:px-8 py-1.5">
            <h2 class="font-bold text-lg text-white leading-tight flex items-center gap-2">
                <x-heroicon-s-pencil-square class="w-4 h-4 text-[#3b82f6]" />
                {{ __('Edit Transaction') }}
            </h2>
        </div>
    </x-slot>

    <!-- Pembungkus Padat & Konsisten -->
            ...
            <div class="bg-[#0f172a] border border-gray-800 rounded-2xl overflow-hidden shadow-lg shadow-black/20">
                <div class="p-5 sm:p-6">
                    <h3 class="text-sm font-bold text-white mb-5 flex items-center">
                        <x-heroicon-s-document-text class="w-4 h-4 mr-1.5 text-[#3b82f6]" />
                    ...
                    <form method="POST" action="{{ route('transactions.update', $transaction) }}" class="space-y-4">
                        ...
                        <div>
                            ...
                            <x-text-input id="amount" class="block mt-1.5 w-full bg-[#11141d] border-gray-700 text-white text-xs focus:border-[#3b82f6] focus:ring-[#3b82f6] rounded-xl px-3 py-2 shadow-sm focus:outline-none" type="number" step="0.01" name="amount" :value="old('amount', $transaction->amount)" required autofocus />
                            <x-input-error :messages="$errors->get('amount')" class="mt-1.5 text-xs text-rose-400" />
                        </div>

                        <!-- Baris Grid: Currency & Type -->
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <select id="currency" name="currency" class="block mt-1.5 w-full bg-[#11141d] border-gray-700 text-white text-xs focus:border-[#3b82f6] focus:ring-[#3b82f6] rounded-xl px-3 py-2 shadow-sm focus:outline-none" required>
                                    @foreach ($currencies as $currency)
                                        <option value="{{ $currency }}" {{ old('currency', $transaction->currency) == $currency ? 'selected' : '' }} class="bg-[#0f172a]">
                                            {{ $currency }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select id="type" name="type" class="block mt-1.5 w-full bg-[#11141d] border-gray-700 text-white text-xs focus:border-[#3b82f6] focus:ring-[#3b82f6] rounded-xl px-3 py-2 shadow-sm focus:outline-none" required>
                                    <option value="expense" {{ old('type', $transaction->type) == 'expense' ? 'selected' : '' }} class="bg-[#0f172a]">{{ __('Expense') }}</option>
                                    <option value="income" {{ old('type', $transaction->type) == 'income' ? 'selected' : '' }} class="bg-[#0f172a]">{{ __('Income') }}</option>
                                </select>
                        ...
                        <div>
                            ...
                            <select id="category_id" name="category_id" class="block mt-1.5 w-full bg-[#11141d] border-gray-700 text-white text-xs focus:border-[#3b82f6] focus:ring-[#3b82f6] rounded-xl px-3 py-2 shadow-sm focus:outline-none">
                                ...
                                @foreach ($categories as $category)
                                    <option value="{{ $category->id }}" {{ old('category_id', $transaction->category_id) == $category->id ? 'selected' : '' }} class="bg-[#0f172a]">
                        ...
                        <div>
                            ...
                            <x-text-input id="transaction_date" class="block mt-1.5 w-full bg-[#11141d] border-gray-700 text-white text-xs focus:border-[#3b82f6] focus:ring-[#3b82f6] rounded-xl px-3 py-2 shadow-sm focus:outline-none" type="date" name="transaction_date" :value="old('transaction_date', $transaction->transaction_date->format('Y-m-d'))" required />
                        ...
                        <div>
                            ...
                            <textarea id="description" class="block mt-1.5 w-full bg-[#11141d] border-gray-700 text-white text-xs focus:border-[#3b82f6] focus:ring-[#3b82f6] rounded-xl px-3 py-2.5 shadow-sm focus:outline-none h-20 min-h-[40px]" name="description">{{ old('description', $transaction->description) }}</textarea>
