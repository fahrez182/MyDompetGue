<x-app-layout>
    <!-- Header Minimalis Sesuai Tema Dashboard -->
    <x-slot name="header">
        <div class="max-w-xl mx-auto px-4 sm:px-6 lg:px-8 py-1.5">
            <h2 class="font-bold text-lg text-white leading-tight flex items-center gap-2">
                <x-heroicon-s-plus-circle class="w-4 h-4 text-amber-400" />
            ...
            <div class="bg-[#0f172a] border border-gray-800 rounded-2xl overflow-hidden shadow-lg shadow-black/20">
                <div class="p-5 sm:p-6">
                    <h3 class="text-sm font-bold text-white mb-5 flex items-center">
                        ...
                        {{ __('Automation Schedule Details') }}
                    </h3>
                    ...
                    <form method="POST" action="{{ route('recurring-transactions.store') }}" class="space-y-4">
                        ...
                        <div>
                            ...
                            <x-text-input id="description" name="description" type="text" class="block mt-1.5 w-full bg-[#11141d] border-gray-700 text-white text-xs focus:border-[#3b82f6] focus:ring-[#3b82f6] rounded-xl px-3 py-2 shadow-sm focus:outline-none" :value="old('description')" required autofocus placeholder="e.g., Netflix Subscription, Rent Bill" />
                        ...
                        <div class="grid grid-cols-3 gap-4">
                            ...
                            <div class="col-span-2">
                                ...
                                <x-text-input id="amount" name="amount" type="number" step="0.01" class="block mt-1.5 w-full bg-[#11141d] border-gray-700 text-white text-xs focus:border-[#3b82f6] focus:ring-[#3b82f6] rounded-xl px-3 py-2 shadow-sm focus:outline-none" :value="old('amount')" required />
                            ...
                            <div>
                                ...
                                <select id="currency" name="currency" class="block mt-1.5 w-full bg-[#11141d] border-gray-700 text-white text-xs focus:border-[#3b82f6] focus:ring-[#3b82f6] rounded-xl px-3 py-2 shadow-sm focus:outline-none" required>
                                    @foreach ($currencies as $currency)
                                        <option value="{{ $currency }}" {{ old('currency', Auth::user()->base_currency ?? 'IDR') == $currency ? 'selected' : '' }} class="bg-[#0f172a]">
                                            {{ $currency }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <select id="type" name="type" class="block mt-1.5 w-full bg-[#11141d] border-gray-700 text-white text-xs focus:border-[#3b82f6] focus:ring-[#3b82f6] rounded-xl px-3 py-2 shadow-sm focus:outline-none" required>
                                    <option value="expense" {{ old('type') == 'expense' ? 'selected' : '' }} class="bg-[#0f172a]">{{ __('Expense (Pengeluaran)') }}</option>
                                    <option value="income" {{ old('type') == 'income' ? 'selected' : '' }} class="bg-[#0f172a]">{{ __('Income (Pemasukan)') }}</option>
                                </select>
                        ...
                        <div>
                            ...
                            <select id="frequency" name="frequency" class="block mt-1.5 w-full bg-[#11141d] border-gray-700 text-white text-xs focus:border-[#3b82f6] focus:ring-[#3b82f6] rounded-xl px-3 py-2 shadow-sm focus:outline-none" required>
                                <option value="daily" {{ old('frequency') == 'daily' ? 'selected' : '' }} class="bg-[#0f172a]">{{ __('Daily (Harian)') }}</option>
                                <option value="weekly" {{ old('frequency') == 'weekly' ? 'selected' : '' }} class="bg-[#0f172a]">{{ __('Weekly (Mingguan)') }}</option>
                                <option value="monthly" {{ old('frequency', 'monthly') == 'monthly' ? 'selected' : '' }} class="bg-[#0f172a]">{{ __('Monthly (Bulanan)') }}</option>
                                <option value="yearly" {{ old('frequency') == 'yearly' ? 'selected' : '' }} class="bg-[#0f172a]">{{ __('Yearly (Tahunan)') }}</option>
                            </select>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <x-text-input id="start_date" name="start_date" type="date" class="block mt-1.5 w-full bg-[#11141d] border-gray-700 text-white text-xs focus:border-[#3b82f6] focus:ring-[#3b82f6] rounded-xl px-3 py-2 shadow-sm focus:outline-none" :value="old('start_date')" required />
                            ...
                            <div>
                                ...
                                <x-text-input id="end_date" name="end_date" type="date" class="block mt-1.5 w-full bg-[#11141d] border-gray-700 text-white text-xs focus:border-[#3b82f6] focus:ring-[#3b82f6] rounded-xl px-3 py-2 shadow-sm focus:outline-none" :value="old('end_date')" />
