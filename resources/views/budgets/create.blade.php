                ...
                <div class="p-5 sm:p-6">
                    ...
                    <form method="POST" action="{{ route('budgets.store') }}" class="space-y-4">
                            ...
                            <div>
                                ...
                                <select id="wallet_id" name="wallet_id" class="block mt-1.5 w-full bg-[#11141d] border-gray-700 text-white text-xs focus:border-[#3b82f6] focus:ring-[#3b82f6] rounded-xl px-3 py-2 shadow-sm focus:outline-none">
                                    ...
                                    @foreach ($wallets as $wallet)
                                        <option value="{{ $wallet->id }}" {{ old('wallet_id') == $wallet->id ? 'selected' : '' }} class="bg-[#0f172a]">
                        ...
                        <div>
                            ...
                            <x-text-input id="amount" name="amount" type="number" step="0.01" class="block mt-1.5 w-full bg-[#11141d] border-gray-700 text-white text-xs focus:border-[#3b82f6] focus:ring-[#3b82f6] rounded-xl px-3 py-2 shadow-sm focus:outline-none" :value="old('amount')" required autofocus />
                        ...
                        <div>
                            ...
                            <select id="currency" name="currency" class="block mt-1.5 w-full bg-[#11141d] border-gray-700 text-white text-xs focus:border-[#3b82f6] focus:ring-[#3b82f6] rounded-xl px-3 py-2 shadow-sm focus:outline-none" required>
                                ...
                                @foreach ($currencies as $currency)
                                    <option value="{{ $currency }}" {{ old('currency', 'USD') == $currency ? 'selected' : '' }} class="bg-[#0f172a]">
                                        {{ $currency }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select id="period" name="period" class="block mt-1.5 w-full bg-[#11141d] border-gray-700 text-white text-xs focus:border-[#3b82f6] focus:ring-[#3b82f6] rounded-xl px-3 py-2 shadow-sm focus:outline-none" required>
                                <option value="monthly" {{ old('period') == 'monthly' ? 'selected' : '' }} class="bg-[#0f172a]">{{ __('Monthly') }}</option>
                                <option value="yearly" {{ old('period') == 'yearly' ? 'selected' : '' }} class="bg-[#0f172a]">{{ __('Yearly') }}</option>
                            </select>
